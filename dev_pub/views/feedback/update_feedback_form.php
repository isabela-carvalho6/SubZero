<!DOCTYPE html>
<html lang="pt-BR"> 
<head>
    <meta charset="UTF-8"> 
    <meta http-equiv="X-UA-Compatible" content="IE=edge"> 
    <meta name="viewport" content="width=device-width, initial-scale=1.0"> 
    <title>Atualizar Feedback</title> 
</head>
<body>

    <h1>Atualizar Feedback</h1>

    <form action="/dev_pub/update-feedback" method="POST">
 
        <input type="hidden" name="id" value="<?php echo $feedbackInfo['id']; ?>">

        <label for="titulo">Título:</label>
        <input type="text" id="titulo" name="titulo" value="<?php echo $feedbackInfo['titulo']; ?>" required><br><br>

        <label for="descricao">Descrição:</label>
        <input type="text" id="descricao" name="descricao" value="<?php echo $feedbackInfo['descricao']; ?>" required><br><br>

        <label for="data_feedback">Data:</label>
        <input type="date" id="date_feedback" name="data_feedback" value="<?php echo $feedbackInfo['data_feedback']; ?>" required><br><br>

        <input type="submit" value="Atualizar Feedback">
    </form>

    <a href="/dev_pub/list-feedback">Voltar para os feedbacks</a>


</body>
</html>
