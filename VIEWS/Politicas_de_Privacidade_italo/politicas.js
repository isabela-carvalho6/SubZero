...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Animar data de atualização
    const updateDate = document.querySelector('.container p strong');
    if (updateDate && updateDate.textContent.includes('atualização')) {
        const dateParagraph = updateDate.parentElement;
        dateParagraph.classList.add('update-date');
        dateParagraph.style.opacity = '0';
        dateParagraph.style.transform = 'translateY(-10px)';
        dateParagraph.style.transition = 'all 0.8s ease-out 0.9s';
    ...
    if (containerContent) {
        ...
        sections.forEach((section, index) => {
            ...
            // ícone removido (sem emojis) — manter apenas o conteúdo da seção
    ...
    // Indicador de progresso de leitura removido

    // Contador de seções removido conforme solicitado
});
...
window.addEventListener('scroll', () => {
    if (!ticking) {
        window.requestAnimationFrame(() => {
            ...
            if (container) {
                if (scrolled > 100) {
                    container.style.boxShadow = '0 30px 80px rgba(255, 255, 255, 0.08)';
                } else {
